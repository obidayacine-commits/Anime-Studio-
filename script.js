...
function openImage(src, caption) {
  ...
  if(downloadBtn) {
    const originalSrc = src.includes('imagekit')? src.split('?')[0] : src;
...
// ========== جلب البيانات من ImageKit ==========
async function loadFromImageKit() {
  try {
    // جلب الصور
    const imgRes = await fetch(`${IMAGEKIT_URL}${IMG_FOLDER}/?ik-list=1&limit=1000`);
    const imgFiles = await imgRes.json();
    allImages = imgFiles
     .filter(f => f.type === 'file' && f.name.match(/\.(jpg|jpeg|png|webp|gif)$/i))
     .map(file => ({
        id: file.fileId,
        src: `${IMAGEKIT_URL}${IMG_FOLDER}/${file.name}?tr=w-400,q-70,f-auto`,
        fullSrc: `${IMAGEKIT_URL}${IMG_FOLDER}/${file.name}`,
        caption: file.name.split(".")[0].replace(/[_-]/g, "),
        tags: extractTags(file.name)
      }));

    // جلب الفيديوهات
    const vidRes = await fetch(`${IMAGEKIT_URL}${VID_FOLDER}/?ik-list=1&limit=1000`);
    const vidFiles = await vidRes.json();
    allVideos = vidFiles
     .filter(f => f.type === 'file' && f.name.match(/\.(mp4|webm|mov)$/i))
     .map(file => ({
        id: file.fileId,
        src: `${IMAGEKIT_URL}${VID_FOLDER}/${file.name}`,
        poster: `${IMAGEKIT_URL}${VID_FOLDER}/${file.name}?tr=w-400,q-70,f-auto`,
        caption: file.name.split(".")[0].replace(/[_-]/g, "),
        tags: extractTags(file.name)
      }));

  } catch (error) {
    console.error("خطأ في جلب ملفات ImageKit:", error);
  }
}
...
function extractTags(filename) {
  ...
  if (name.includes("gojo")) tags.push("gojo");
  if (name.includes("sukuna")) tags.push("sukuna");
  if (name.includes("yuji") || name.includes("itadori")) tags.push("yuji");
  if (name.includes("toji")) tags.push("toji");
  if (name.includes("megumi")) tags.push("megumi");
  if (name.includes("choso")) tags.push("choso");
  if (name.includes("suguru") || name.includes("geto")) tags.push("suguru");
  if (name.includes("yuta")) tags.push("yuta");
  return tags;
}

// ========== باقي الكود ==========
document.addEventListener('DOMContentLoaded', async function() {
  // جلب الملفات من ImageKit أولاً
  ...
  function showWelcomeMessage(name) {
    ...
    if (name === 'رجاء' || name === 'Rajaa' || name === 'rajaa') {
      messageText.innerHTML = ` Welcome Rajaa<br> أهلاً بك يا رجاء في موقعي<br> أرجو أن ينال إعجابك <span class="special-name">I❤️ you Rajaa</span> `;
    } else {
      messageText.innerHTML = ` أهلاً بك يا ${name}<br><br> أرجو أن يعجبك الموقع<br> لا تنسى التواصل معنا إذا اردت إضافة فكرة للموقع أو واجهة مشكلة ما `;
    }
    ...
    itemsToRender.forEach(item => {
      ...
      if(currentTab === 'Pictures') {
        ...
        div.querySelector('.fav-btn').addEventListener('click', (e) => {
          e.stopPropagation();
          toggleFavorite(item.id, 'img', item.fullSrc, '', e);
        });
        div.querySelector('img').addEventListener('click', () => {
          openImage(item.fullSrc, item.caption);
        });
